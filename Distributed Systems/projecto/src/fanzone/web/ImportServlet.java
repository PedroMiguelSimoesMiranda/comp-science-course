...
public class ImportServlet extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String imp = request.getParameter( FZConstants.ATTRIB_IMPORT);
			if( imp == null  || imp.trim().length() == 0) {
				request.getRequestDispatcher("/importevent.jsp").forward(request,response);
				return;
			}

			session.setAttribute( FZConstants.ATTRIB_IMPORT, imp);
			...
			List<EventInfo> events = proc.searchEventImport(imp, query);
			...
			session.setAttribute( FZConstants.ATTRIB_EVT_INIMPORT, new Boolean(true));
			session.setAttribute( FZConstants.ATTRIB_EVT_INLISTIMPORT, new Boolean(false));
			
			if( events.size() == 1) {
				session.setAttribute( FZConstants.ATTRIB_EVT_INLISTIMPORT, new Boolean(false));
				session.setAttribute( FZConstants.ATTRIB_CUREVENT, events.get(0));
				request.getRequestDispatcher("/event.jsp").forward(request,response);
			} else {
				session.setAttribute( FZConstants.ATTRIB_EVT_INLISTIMPORT, new Boolean(true));
