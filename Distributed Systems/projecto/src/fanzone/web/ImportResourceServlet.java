...
public class ImportResourceServlet extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			List<ResourceInfo> res = proc.searchResourceImport(imp, query);
					
			session.setAttribute( FZConstants.ATTRIB_RESOURCE_LIST, res);
			session.setAttribute( FZConstants.ATTRIB_RESOURCE_POS, new Integer(0));
			session.setAttribute( FZConstants.ATTRIB_RES_INIMPORT, new Boolean(true));
			session.setAttribute( FZConstants.ATTRIB_RES_INLISTIMPORT, new Boolean(true));
