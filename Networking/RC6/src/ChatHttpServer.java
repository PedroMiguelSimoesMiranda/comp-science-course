		...
		try {
			is = socket.getInputStream();
				...
				if(req[0].equals("GET")){
					...
					Url url = new Url(req[1]);
					
					String newReq = "GET /"+url.getRequest							()+" HTTP/1.0\r\n\r\n";
					String aux = "";

					do{
						aux = HTTPUtilities.readLine(is);
						newReq.concat(aux);
					}while(!aux.isEmpty());
					
					Socket server = new Socket(url.getHost								(), url.getPort(80));
					OutputStream outs = 									socket.getOutputStream();
					InputStream ins = 
						socket.getInputStream();
					outs.write( newReq.getBytes() ) ;
					String serverReply = 									HTTPUtilities.readLine(is);
					
					do{
						aux = HTTPUtilities.readLine(is);
						serverReply.concat(aux);
					}while(!aux.isEmpty());
					
					serverReply.replaceFirst("HTTP/1.1", 						"HTTP/1.0");
					
					socket.getOutputStream().write						(serverReply.getBytes(), 0, 							serverReply.getBytes().length);
