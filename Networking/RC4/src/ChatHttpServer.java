...
=======================================================================
	 ...
	 * GET /chatRoomName HTTP/1.x - obtem pagina
	com conteudo da chat room
	 * POST /chatRoomName HTTP/1.x - adiciona mensagem 
	ao chat room: parametros no 
	 *corpo do pedido HTTP, codificados como 
	...
	void processRequest( Socket socket) {
		...
		//TODO: Completar com codigo para atender uma 				
	//conexao/pedido
		...
		try {
			...
			String[] req = HTTPUtilities.parseHttpRequest(line);
				...
				while(!HTTPUtilities.readLine(is).isEmpty()){}
				//Ler Resto
				byte[] res = 
					"HTTP/1.0 200 OK\r\n\r\n".getBytes();
				socket.getOutputStream().write								(res,0,res.length);
					...
					req = 
					HTTPUtilities.parseHttpHeader(line);
					String msg = "";
					int x = 0,clength = 0;
					while(!line.isEmpty()){
					...
					dumpStream(chat.getFormPageInputStream						(chatroom,prop.getProperty						     ("nickName")),
